...
const Course = {
    ...
    bindEvents: function() {
        ...
        // Ensure all event handlers are properly bound to 'this'
        ...
        this.showCreateCourseForm = this.showCreateCourseForm.bind(this);
    },
    ...
    renderCourseSection: function() {
        ...
        tableContainer.innerHTML = `
            ...
            <div id="course-pagination" class="px-6 py-4 flex justify-center">
        ...
        // Add event listeners
        document.getElementById('create-course-btn').addEventListener('click', this.showCreateCourseForm);
        document.getElementById('course-search-btn').addEventListener('click', this.handleCourseSearch);
        document.getElementById('course-search').addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                this.handleCourseSearch();
            }
        });
        document.getElementById('course-filter-btn').addEventListener('click', this.handleCourseFilter);
    ...
    renderCourses: function(courses) {
        ...
        courses.forEach(course => {
            // Extract credit hours from courseDescription if not directly available
            let creditHours = course.creditHours;
            if (!creditHours && course.courseDescription) {
                // Try to extract credit hours from description if it contains a pattern like "Credit Hours: X"
                const match = course.courseDescription.match(/Credit Hours:\s*(\d+)/i);
                if (match) {
                    creditHours = match[1];
                }
            }
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${creditHours || 'N/A'}</td>
    ...
    /**
     * Filter courses by department and/or semester
    ...
    filterCourses: async function(departmentId, semester) {
        try {
            ...
            const response = await API.course.getFiltered(departmentId, semester);
     ...
     */
    renderCoursePagination: function() {
...
// Initialize course module when DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM loaded, initializing Course module");
    Course.init();
});
