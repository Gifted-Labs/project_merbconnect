...
const Course = {
    ...
    loadCourses: async function() {
        try {
            ...
            // Instead of response.data.content, we have response.data directly
            const courses = response.data || [];
            
            // For pagination, we might need to adjust this if the API doesn't provide totalPages
            // For now, assume we have just one page if we got results
            this.totalPages = courses.length > 0 ? 1 : 0;
    ...
    filterCourses: async function(departmentId, semester) {
        try {
            ...
            if (departmentId && semester) {
                ...
                response = await API.course.getByDepartmentIdAndSemester(departmentId, semester);
            } else if (departmentId) {
                // Filter by department only
                response = await API.course.getByDepartmentId(departmentId);
            } else if (semester) {
            ...
            if (courses.length === 0) {
                ...
                if (departmentId && semester) {
                    message += " for the selected department and semester";
                } else if (departmentId) {
                    message += " for the selected department";
                } else if (semester) {
                    message += " for the selected semester";
                }
                this.renderEmptyCourses(message);
            }
        } catch (error) {
            Utils.hideLoading();
